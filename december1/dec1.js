const initialValue = `ninefourone1
53sevenvvqm
...
const getFirstAndLastNumber = (code) => {
  let value;
  ...
  if (newCode.length === 1) value = newCode[0] + newCode[0] * 10;

  if (newCode.length === 2) value = newCode[0] * 10 + newCode[1];

  if (newCode.length > 2) value = newCode[0] * 10 + newCode[newCode.length - 1];

  return value;
};

const calibratedValue = initialValue
  .split("\n")
  .map((e) => getFirstAndLastNumber(e))
  .reduce((t, c) => t + c);

console.log("Part 1:", calibratedValue); // 54450

// PART 2

const getFirstAndLastNumber2 = (code) => {
  const NUM_REGEX = /(\d)/;
  const WRITTEN_NUM_REGEX =
    /(?:(?:one|two|three|four|five|six|seven|eight|nine|\d))/g;
  const REVERSED_WRITTEN_NUM_REGEX =
    /(?:(?:eno|owt|eerht|ruof|evif|xis|neves|thgie|enin|\d))/g;

  const reversedCode = code.split("").reverse().join("");
  const newCodeFirstNum = [...code.match(WRITTEN_NUM_REGEX)][0];
  const newCodeLastNum = [...reversedCode.match(REVERSED_WRITTEN_NUM_REGEX)][0]
    .split("")
    .reverse()
    .join("");

  const turnWrittenToNum = (e) => {
    if (NUM_REGEX.test(e)) return parseInt(e);
    if (e === "one") return (e = 1);
    if (e === "two") return (e = 2);
    if (e === "three") return (e = 3);
    if (e === "four") return (e = 4);
    if (e === "five") return (e = 5);
    if (e === "six") return (e = 6);
    if (e === "seven") return (e = 7);
    if (e === "eight") return (e = 8);
    if (e === "nine") return (e = 9);
  };

  const value =
    turnWrittenToNum(newCodeFirstNum) * 10 + turnWrittenToNum(newCodeLastNum);

  return value;
};

const calibratedValue2 = initialValue
  .split("\n")
  .map((e) => getFirstAndLastNumber2(e))
  .reduce((t, c) => t + c);

console.log("Part 2:", calibratedValue2); // 54265
